...
output: rmarkdown::github_document #html_vignette
vignette: >
  %\VignetteIndexEntry{enlabel-vignette}
...
`enlabel` allows you to source the dataset for labels with `extract_label_from_data()`. The function accepts as an argument either a path to the .sav file or to an R dataframe with a `variable.labels` attribute. Importing a .sav file with the `foreign` package or using the "Export to R" functionality in LimeSurvey will both create such objects. Unfortunately, this does not work with objects imported with the `haven` package (this means that, in this particular case, `extract_labels_from_data(df)` is not going to work).
